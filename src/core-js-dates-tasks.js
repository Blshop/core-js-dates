...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const sec = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  return sec / 60 / 60 / 24 / 1000 + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const checkPeriod = new Date(date).getTime();
  return start <= checkPeriod && end >= checkPeriod;
}
...
function getWeekNumberByDate(date) {
  ...
  const forth = new Date(current.getFullYear(), 0, 4).getDay();
  const firstWeeklength = 7 - (forth === 0 ? 7 : forth) + 4;
  const endOfWeek = new Date(current.getFullYear(), 0, firstWeeklength);
  if (current <= endOfWeek) return 1;
  return (
    Math.ceil(
      (current.getTime() - endOfWeek.getTime()) / (1000 * 60 * 60 * 24 * 7)
    ) + 1
  );
}
...
function getNextFridayThe13th(date) {
  ...
  while (flag) {
    ...
    if (newDate.getDay() === 5) flag = false;
    month += 1;
  }
  return newDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (start.getTime() <= end.getTime()) {
    for (let i = 0; i < countWorkDays; i += 1) {
      schedule.push(formatDATE(start));
      start.setDate(start.getDate() + 1);
      if (start.getTime() > end.getTime()) return schedule;
    }
